...
class VP_Option
{
	...
	public function __construct(array $configs)
	{
		...
		$configs = array_merge(array(
			...
			'layout'                => 'fixed',
			'page_title'            => esc_html__( 'Vafpress Options', 'rehub-framework' ),
			'menu_label'            => esc_html__( 'Vafpress Options', 'rehub-framework' ),
			'priority'              => 10,
		), $configs);
		...
		else throw new Exception(__( 'Option Key is required', 'rehub-framework' ), 1);
		if(isset($template)) $this->set_template($template);
		else throw new Exception(__( 'Template Array/File is required', 'rehub-framework' ), 1);
		if(isset($page_slug)) $this->set_page_slug($page_slug);
		else throw new Exception(__( 'Page Slug is required', 'rehub-framework' ), 1);
	...
	{
		VP_WP_Util::admin_notice(__("Development Mode is Active, options' values won't be saved into database.", 'rehub-framework'), false);
	}
	...
	function ajax_import_option()
	{
		...
		if($result['status'])
		{
			...
			{
				$result['status']  = false;
				$result['message'] = esc_html__("Can not be empty", 'rehub-framework');
			}
			else
			{
				...
				{
					$result['status']  = false;
					$result['message'] = esc_html__("Invalid data", 'rehub-framework');
				}
			}
		}
	...
	function ajax_export_option()
	{
		...
		if($result['status'])
		{
			...
			$result = array(
				'status' => true,
				'message'=> esc_html__("Successful", 'rehub-framework'),
				'option' => $sr_options,
			);
		}
	...
	function vp_verify_nonce()
	{
		...
		{
			$result['status']  = true;
			$result['message'] = esc_html__("Successful", 'rehub-framework');	
		}
		else
		{
			$result['status']  = false;
			$result['message'] = esc_html__("Unverified Access", 'rehub-framework');
		}
		return $result;
	}
	...
	public function init_options_set()
	{
		...
		if( is_string($this->get_template()) and is_file($this->get_template()) )
			...
			throw new Exception(__( 'Invalid template supplied', 'rehub-framework' ), 1);
		...
		if( $this->use_util_menu() )
		{
			...
			$util_menu->set_title(__('Utility', 'rehub-framework'));
			$util_menu->set_name('menu_util');
			$util_menu->set_icon('rhicon rhi-cogs');

			// setup restore default section
			$restore_section = new VP_Option_Control_Group_Section();
			$restore_section->set_title(__('Restore Default', 'rehub-framework'));
			...
			$exim_section->set_title(__('Export/Import', 'rehub-framework'));
